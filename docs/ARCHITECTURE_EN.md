...
-> user selects or confirms a candidate model
-> final stage writes final output and source code
```

The summary stage enumerates candidate combinations, such as control-variable subsets and standard-error choices. The resulting `combination_summary.csv` is a decision aid, not an automatic final conclusion.
...
- `scripts/workflow/`: data profiling and plan compilation.
