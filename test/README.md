...
The demo env test uses `/Users/daydream/Desktop/demo.dta` when available.

The env test shape is:
...
For env parity, compare stage 1 outputs and stage 3 Word outputs for the same selected `cv_idx` and `vce_idx`. Stage 2 source files are env-specific and should be checked for existence and reproducibility, not byte-level equality.
