...
/*
Summary stage (stata env): regression args -> combination summary.

IN:  starlane_* config globals (generated by workflow/stata_config.py)
OUT: combination_summary.csv in $STARLANE_EXPORT
...
Performance: No data reload per iteration; rob vars created once; cv subsets in memory; quietly + no est store.
...
local data_path_arg "$starlane_data_path"
...
local het_disc_arg "$starlane_het_disc"
local het_disc_vals_arg "$starlane_het_disc_vals"
local rob_vars_arg "$starlane_rob_vars"
local ln_y_arg "$starlane_ln_y"
local ln_x_arg "$starlane_ln_x"
...
if "`data_path_arg'" == "" | "`y_arg'" == "" | "`x_arg'" == "" | "`cv_arg'" == "" | "`panelvar_arg'" == "" | "`timevar_arg'" == "" {
	di as error "Usage: set starlane_* config globals, then: do build_summary.do (see workflow/stata_config.py)"
	exit 198
}

// cluster_var = panelvar, cluster_var2 = timevar (one-way / two-way clustering identifiers)
local panelvar_name = trim("`panelvar_arg'")
local cluster_var "`panelvar_name'"
local cluster_var2 = trim("`timevar_arg'")

capture confirm file "`data_path_arg'"
if _rc {
	di as error "Input file not found: `data_path_arg'"
	exit 601
}

// Load data: .dta, .xlsx, .xls, or .csv (first row as headers for non-dta)
local input_ext = lower(substr("`data_path_arg'", -4, .))
local input_ext5 = lower(substr("`data_path_arg'", -5, .))
if "`input_ext'" == ".dta" {
	use "`data_path_arg'", clear
}
else if "`input_ext'" == ".csv" {
	import delimited "`data_path_arg'", clear varnames(1) case(preserve)
}
else if "`input_ext'" == ".xls" | "`input_ext5'" == ".xlsx" {
	import excel "`data_path_arg'", firstrow clear case(preserve)
}
else {
	di as error "Unsupported input format: `data_path_arg'"
...
if _rc == 0 {
	tempvar __panel_numeric_probe
	capture gen double `__panel_numeric_probe' = real(trim(`panelvar_name'))
	...
	if _rc != 0 {
		...
		capture count if trim(`panelvar_name') != "" & missing(`__panel_numeric_probe')
		if _rc != 0 {
			local panelvar_needs_group 1
		}
		else if r(N) > 0 {
	...
	capture drop `__panel_numeric_probe'
}
...
// Parse meds, mods, het_disc, het_disc_vals, rob_vars, iv
global meds ""
...
global het_disc ""
local het_disc_raw = trim("`het_disc_arg'")
if "`het_disc_raw'" != "" {
	local het_disc_parsed = subinstr("`het_disc_raw'", "|", " ", .)
	global het_disc "`het_disc_parsed'"
	// Validate: het_disc must be variable names (e.g. SOE|Region), not parameter names
	foreach _v of global het_disc {
		if regexm(`"`_v'"', "^_") | regexm(`"`_v'"', "_arg$") {
			di as error "Invalid het_disc: `_v'. Pass variable names (e.g. SOE|Region), not parameter names. Values belong in starlane_het_disc_vals (e.g. SOE:1;0)."
			exit 198
		}
	}
}

global het_disc_vals "`het_disc_vals_arg'"
if regexm(`"$het_disc_vals"', "^_[a-z_]+_arg$") {
	di as error "Invalid het_disc_vals: pass values (e.g. SOE:1;0|Region:East;West), not param names."
	exit 198
}
...
// ln_y, ln_x: "1"/"是"/"" = do ln robustness (default); "0"/"否" = skip
local ln_y_val = trim(lower("`ln_y_arg'"))
local ln_x_val = trim(lower("`ln_x_arg'"))
local ln_y_auto 0
local ln_x_auto 0
if inlist("`ln_y_val'", "", "1", "是", "true", "yes") {
	local ln_y_auto 1
}
if inlist("`ln_x_val'", "", "1", "是", "true", "yes") {
	local ln_x_auto 1
}
global ln_y_auto `ln_y_auto'
global ln_x_auto `ln_x_auto'
...
// Create rob vars once (no reload per iteration)
// When ln_y=是 or ln_x=是, do ln(y)/ln(x) robustness; when 否, skip
...
if "$mods" != "" {
	foreach x of global x {
		capture drop std_`x'
		quietly egen std_`x' = std(`x')
	}
	foreach mod_var of global mods {
		capture drop std_`mod_var'
		quietly egen std_`mod_var' = std(`mod_var')
	}
}
...
forvalues cv_idx = `loop_start'/`loop_end' {
	...
	if "$het_disc" != "" local sample_pool_vars "`sample_pool_vars' $het_disc"
	...
	forvalues vce_idx = 0/3 {
		...
		// Robustness (skip unless at least one baseline-cv is significant AND has correct direction), order matches the canonical ModelPlan: alt_x, alt_y, ln_x, ln_y, lag, year
		...
		if "$rob_alt_x" != "" {
			foreach y of global y {
				foreach x of global rob_alt_x {
					...
					capture quietly reghdfe `y' `x' $cv if __base_sample, absorb($timevar $panelvar) $vce
					if _rc == 0 {
						_post_coef scorehandle "`vce_suffix'" "rob_alt_x" "`x'"
					}
					else {
						post scorehandle ("`vce_suffix'") ("rob_alt_x") ("`y'") ("`x'") (.) (.) (0)
					}
				}
			}
		}
		if "$rob_alt_y" != "" {
			foreach y of global rob_alt_y {
				foreach x of global x {
					...
					capture quietly reghdfe `y' `x' $cv if __base_sample, absorb($timevar $panelvar) $vce
					if _rc == 0 {
						_post_coef scorehandle "`vce_suffix'" "rob_alt_y" "`x'"
					}
					else {
						post scorehandle ("`vce_suffix'") ("rob_alt_y") ("`y'") ("`x'") (.) (.) (0)
					}
				}
			}
		}
		if "`rob_ln_x_use'" != "" {
			foreach y of global y {
				foreach x of local rob_ln_x_use {
					...
					capture quietly reghdfe `y' `x' $cv if __base_sample, absorb($timevar $panelvar) $vce
					if _rc == 0 {
						_post_coef scorehandle "`vce_suffix'" "rob_ln_x" "`x'"
					}
					else {
						post scorehandle ("`vce_suffix'") ("rob_ln_x") ("`y'") ("`x'") (.) (.) (0)
					}
				}
			}
		}
		if "`rob_ln_y_use'" != "" {
			foreach y of local rob_ln_y_use {
				foreach x of global x {
					...
					capture quietly reghdfe `y' `x' $cv if __base_sample, absorb($timevar $panelvar) $vce
					if _rc == 0 {
						_post_coef scorehandle "`vce_suffix'" "rob_ln_y" "`x'"
					}
					else {
						post scorehandle ("`vce_suffix'") ("rob_ln_y") ("`y'") ("`x'") (.) (.) (0)
					}
				}
			}
		}
			...
			foreach p of global rob_lag_periods {
				foreach y of global y {
					foreach x of global x {
						...
						if _rc == 0 {
							_post_coef scorehandle "`vce_suffix'" "rob_lag" "l`p'.`x'"
						}
						else {
							post scorehandle ("`vce_suffix'") ("rob_lag") ("`y'") ("l`p'.`x'") (.) (.) (0)
						}
					}
				}
			}
		...
		if "$rob_year_range_cond" != "" {
			foreach y of global y {
				foreach x of global x {
					...
					if _rc == 0 {
						_post_coef scorehandle "`vce_suffix'" "rob_year" "`x'"
					}
					else {
						post scorehandle ("`vce_suffix'") ("rob_year") ("`y'") ("`x'") (.) (.) (0)
					}
				}
			}
		}

		// IV (2SLS two stages: stage1=iv coef in x~iv+cv, stage2=x coef in y~x_hat+cv)
		if "$iv" != "" {
			foreach y of global y {
				foreach x of global x {
		...
		foreach med_var of global meds_run {
			foreach y of global y {
				foreach x of global x {
					...
					capture quietly reghdfe `y' `x' $cv if __base_sample, absorb($timevar $panelvar) $vce
					if _rc == 0 {
						_post_coef scorehandle "`vce_suffix'" "med_`med_var'" "`x'"
					}
					else {
						post scorehandle ("`vce_suffix'") ("med_`med_var'") ("`y'") ("`x'") (.) (.) (0)
					}
				}
			}
			foreach x of global x {
				...
				if _rc == 0 {
					...
					post scorehandle ("`vce_suffix'") ("med_`med_var'") ("`med_var'") ("`x'") (.) (.) (0)
				}
			}
		}
		...
		if "$mods" != "" {
			foreach mod_var of global mods {
				foreach y of global y {
					foreach x of global x {
						local ++m
						capture quietly reghdfe `y' c.std_`x'##c.std_`mod_var' $cv if __base_sample, absorb($timevar $panelvar) $vce
						if _rc == 0 {
							local coef = _b[c.std_`x'#c.std_`mod_var']
							local se   = _se[c.std_`x'#c.std_`mod_var']
							local dep  = "`e(depvar)'"
							_calc_effective_stars `coef' `se'
							post scorehandle ("`vce_suffix'") ("mod_`mod_var'") ("`dep'") ("interaction") (`coef') (`se') (`effective_stars')
						}
						else {
							post scorehandle ("`vce_suffix'") ("mod_`mod_var'") ("`y'") ("interaction") (.) (.) (0)
						}
					}
				}
			}
		...
		* Same (y,x) grouped; within each (y,x), all group values adjacent (align with Stata final source generator)
		foreach group_var of global het_disc {
			_load_discrete_values_for_var `"$het_disc_vals"' "`group_var'" discrete_vals
			...
			foreach y of global y {
				foreach x of global x {
					forvalues discrete_idx = 1/`discrete_vals_count' {
						...
						local section_name "het_disc_`group_var'_`discrete_idx'"
		...
		// Compute score and build row from scoreaccum (coef+stars cells)
		tempname S_score
		preserve
		scalar `S_score' = 0
